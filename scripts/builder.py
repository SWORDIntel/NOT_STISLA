...
def detect_cpu() -> dict:
    ...
    if len(all_core_flags) > 1:
        ...
        if len(unique_flag_sets) > 1:
            # Hybrid CPU detected — use intersection for baseline
    ...
    if len(all_core_flags) > 1:
        ...
        if len(unique_flag_sets) > 1:
            feat["hybrid"] = True
            # Identify P-cores (have AVX-512 or more features) vs E-cores
...
def install_opt(dest: Path) -> None:
    ...
    for sub in ("bin", "lib", Path("include") / "keystone"):
    ...
    if lib.exists():
        if safe_copy(lib, dest / "lib" / "libkeystone.so"):
    ...
    for b in (ROOT / "bin").glob("test_*"):
        if safe_copy(b, dest / "bin" / b.name):
            bin_count += 1
...
def main() -> None:
    parser = argparse.ArgumentParser(description="KEYSTONE builder", add_help=False)
    ...
    parser.add_argument("--goal", default=None, help="Build goal (all/lib/tests/benchmarks)")
    ...
    # Interactive menus only if --modify flag or unusual settings needed
    if args.modify:
        while True:
    ...
    if review == "m" and not args.modify:
        args.modify = True
        # Fall through to interactive menus below by re-entering the modify block
        # Simpler: just restart with the flag
